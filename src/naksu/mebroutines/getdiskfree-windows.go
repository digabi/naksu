...
func getDiskFreeWindows(path string) (uint64, error) {
	...
	if len(patternResult) < 2 {
		log.LogDebug(fmt.Sprintf("Could not detect drive letter from path: %s", path))
	...
	if err != nil {
		log.LogDebug(fmt.Sprintf("getDiskFreeWindows() could not make WMI query (%s): %s", wmiQuery, fmt.Sprint(err)))
