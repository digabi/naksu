...
func GetServer(newVagrantfilePath string) {
	...
	if newVagrantfilePath == "" {
		...
		if errDownload != nil {
			log.LogDebug(fmt.Sprintf("Download failed: %v", errDownload))
	...
	if err != nil {
		log.LogDebug(fmt.Sprintf("Copying failed, error: %v", err))
